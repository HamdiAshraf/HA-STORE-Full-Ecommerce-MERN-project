...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    try {
      await logoutUser().unwrap(); // Log out the user
  ...
  const adminDropDownMenu = [
    ...
    { label: "Add Post", path: "/dashboard/add-post" },
  ];
  ...
  const dropDownMenus = user && user.role === "admin" ? adminDropDownMenu : userDropDownMenu;
    ...
    <header className="fixed-nav-bar w-nav">
      <nav className="max-w-screen-2xl mx-auto px-4 flex justify-between items-center">
        ...
        <div className="nav__logo">
          <Link to="/">HA STORE<span>.</span></Link>
        </div>
